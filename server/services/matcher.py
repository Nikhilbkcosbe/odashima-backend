...
class Matcher:
    ...
    def compare_items(self, pdf_items: List[TenderItem], excel_items: List[TenderItem]) -> ComparisonSummary:
        ...
        """
        logger.info(f"Starting comparison: {len(pdf_items)} PDF items vs {len(excel_items)} Excel items")
        ...
        logger.info(f"Normalized: {len(pdf_normalized)} PDF items, {len(excel_normalized)} Excel items")
        ...
        for pdf_idx, (pdf_key, pdf_item) in enumerate(pdf_normalized.items()):
            logger.debug(f"Processing PDF item {pdf_idx + 1}/{len(pdf_normalized)}: {pdf_key[:50]}...")
            ...
            if comparison_result.excel_item and comparison_result.status in ["OK", "QUANTITY_MISMATCH"]:
                excel_normalized_key = self.normalizer.normalize_item(comparison_result.excel_item.item_key)
                matched_excel_keys.add(excel_normalized_key)
        
        # Find extra items in Excel (not requested but included for completeness)
        extra_excel_results = self._find_extra_excel_items(excel_normalized, matched_excel_keys)
        ...
        for item in items:
            ...
            if normalized_key in normalized:
                ...
                while normalized_key in normalized:
                    ...
                    counter += 1
                logger.warning(f"Duplicate key found in {source}: {original_key} -> {normalized_key}")
    ...
    def _compare_single_pdf_item(self, pdf_key: str, pdf_item: TenderItem, 
        ...
        fuzzy_match = self._find_fuzzy_match(pdf_key, excel_normalized, matched_excel_keys)
    ...
    def _find_fuzzy_match(self, pdf_key: str, excel_normalized: Dict[str, TenderItem], 
        ...
        available_excel_keys = [key for key in excel_normalized.keys() if key not in matched_excel_keys]
    ...
    def _create_comparison_result(self, pdf_item: TenderItem, excel_item: TenderItem, 
        ...
        if abs(quantity_diff) < quantity_tolerance:
            ...
            logger.debug(f"Perfect match: {pdf_item.item_key[:30]}... (quantities match)")
        else:
    ...
    def _find_extra_excel_items(self, excel_normalized: Dict[str, TenderItem], 
        ...
        for excel_key, excel_item in excel_normalized.items():
            if excel_key not in matched_excel_keys:
                logger.info(f"Extra item in Excel (not in PDF): {excel_item.item_key}")
    ...
    def _generate_summary(self, results: List[ComparisonResult]) -> ComparisonSummary:
        ...
        quantity_mismatches = sum(1 for r in results if r.status == "QUANTITY_MISMATCH")
    ...
    def _log_summary(self, summary: ComparisonSummary):
        ...
        if summary.missing_items > 0:
            ...
            if summary.missing_items > 10:
                logger.warning(f"... and {summary.missing_items - 10} more missing items")
        ...
        if summary.quantity_mismatches > 0:
            ...
            if summary.quantity_mismatches > 5:
                logger.warning(f"... and {summary.quantity_mismatches - 5} more quantity mismatches")
